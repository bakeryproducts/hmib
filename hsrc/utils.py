...
def filter_ban_str_in_name(s: str, bans: List[str]):
    return any([(b in str(s)) for b in bans])
...
def polyg_to_mask(polyg: np.ndarray, wh: Tuple[int, int],
...
def json_record_to_poly(record: Dict) -> List[geometry.Polygon]:
    ...
    if num_polygons == 1:  # Polygon
        ...
        list_coords = [
            record['geometry']['coordinates'][i][0]
...
def save_models(d, postfix, output_folder):
    for k, v in d.items():
        torch.save(
            v.state_dict(),
            output_folder / os.path.join("models", f"{k}_{postfix}.pkl"))
...
def get_tiff_block(ds, x, y, w, h=None, bands=3):
    if h is None: h = w
    return ds.read(list(range(1, bands + 1)),
...
def save_tiff_uint8_single_band(img, path, bits=1):
    ...
    if img.max() <= 1.:
        print(f"Warning: saving tiff with max value is <= 1, {path}")
    h, w = img.shape
...
def get_polygons_by_type(anot_structs_json: Dict,
                         name: str) -> List[geometry.Polygon]:
    polygons = []
